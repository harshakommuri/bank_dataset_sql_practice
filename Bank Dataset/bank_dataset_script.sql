...
select count(*) from bank_details where age<40

select * from ineuron_fsda.bank_details limit 5
...
# 1.Find out average of balance
select avg(balance) from bank_details

# 2.Who has minimum of balance?
...
# 4.Prepare a list of persons who are having loans??
select * from bank_details where loan = 'yes'
select count(*) from bank_details where loan = 'yes'

# 5.Find out average balance of the people whose job role is admin??
select avg(balance) from bank_details where job = 'admin.'

# 6.Find out the records where education is primarty and person is jobless??
select * from bank_details where (education = 'primary' and job = 'unknown')

# 7. find the list of jobless persons who are less then age of 45??
select * from bank_details where job = 'unknown' and age<45

# 8. Find the list of records where balance is in negative??
select * from bank_details where balance<0

# 9. Find out all the records who doesnt have house at all along with balance??
select balance from bank_details where housing = 'no' 


#To avoid excuating the query again and again we use procedures. Same as functions in progremming languages. In MySql we have functions as well.
USE ineuron_fsda
...
DELIMITER &&
create procedure avg_bal_jobtype(IN var varchar(30))
BEGIN
	select avg(balance) from bank_details where job = var;
END &&

call select_rec()
call avg_bal_jobtype('admin.')


DELIMITER &&
create procedure bank_details_condioned(IN var1 varchar(30), var2 varchar(30))
BEGIN
	select * from bank_details where (education = var1 and job = var2);
END &&
call bank_details_condioned('secondary', 'technician')


#Creating View, we use view keyword to create the subset data of the orginal data//
